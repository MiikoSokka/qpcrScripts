...
getDeltaDeltaCq <- function(originlist, nonoriginlist, sample, control){
  ...
  ori.C <- getOriginCqs(originlist, control)
  nonOri.C <- getNonOriginCqs(nonoriginlist, control)
  
  deltaCq.X <- getDeltaCqs(ori.X, nonOri.X)
  deltaCq.C <- getDeltaCqs(ori.C, nonOri.C)
  
  deltaDeltaCq <- cbind(deltaCq.X, deltaCq.C)
  
#  if ( deltaDeltaCq[i,c(1,2)] == deltaDeltaCq[i,c(4,5)] ){ # check that the well number information matches
    deltaDeltaCq <- deltaDeltaCq[,c(1:3,6)]
    deltaDeltaCq <- mutate(deltaDeltaCq, ddCq = 2^-(dCq - dCq.1))
    deltaDeltaCq <- deltaDeltaCq[,c(1,2,5)]
    colnames(deltaDeltaCq) <- c("origin", "normalizer", "ddCq")
    write.table(deltaDeltaCq, file.path("output_ddCq/deltaDeltaCq.csv"),
                sep =',', row.names = FALSE)
...
list.options <- list( 
  make_option(c("-i", "--input"), type="character", default = "output_Cq/table_Cq.csv",
              ...
              metavar="./outputCq/table_Cq.csv"),
  make_option(c("-d", "--detectors"), type="character", default = "/Users/Miiko/software/own/qpcrScripts/yeastORI_detector_table.csv",
  ...
  make_option(c("-c", "--control"), type="character",
              help="Specify the name of the calibrator, often designated as C.
              Sample X and calibrator/control C form a matching pair, where the
              enrichment in X is compared to enrichment in C",
              metavar="Calibrator-C")
  )
...
sampleC.name <- as.character(opt$control)
...
#input.file <- "/Users/Miiko/Documents/projects/ns-seq_method-dev/troubleshooting/2018-12-05_fullNS/qpcr/output_Cq"
# detector.file <- "detector_table.csv"
